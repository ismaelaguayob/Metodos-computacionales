...
-   Hacer que el total_results se obtenga antes de comenzar a iterar

-   Hacer el select dentro de la iteración, definiendo las clases de las variables y poniendo en NA los category_primary.name y second cuando estas columnas no existan.
...
# Falta, hacer que ahí dentro vaya search_query y además un vector de palabras que se agreguen para la búsqueda, palabras de interés
pattern <- "(?i)\\b(inteligencia artificial|IA)\\b" 

# Expresión regular optimizada para buscar "inteligencia artificial" o "IA"
all_data <- all_data %>%
  mutate(
...
# Identificamos las filas que NO contienen la búsqueda
indices_no_match <- which(!grepl(search_query, all_data$post_content, ignore.case = TRUE))
